...
import { getUserProfile, getTokenUsage } from './state/conversation.js';
...
app.get('/usage/:tenantId', async (req, res) => {
  const usage = await getTokenUsage(req.params.tenantId).catch(() => null);
  if (!usage) {
    res.status(404).json({ error: 'No usage data found for this tenant' });
    return;
  }
  res.json(usage);
});
...
async function notifyAdmin(message: string): Promise<void> {
  ...
  sendMessage(ADMIN_NOTIFY_NUMBER, message).catch(err =>
    console.warn('[notify] Failed to send admin notification:', err)
  );
}
...
app.post(
  ...
  createWebhookHandler(
    async (from, text, mediaUrl) => {
      const start = Date.now();
      console.log(`[main] Message from ${from}`);

      // Notify admin on first message from a new sender
      const existingProfile = await getUserProfile(from).catch(() => null);
      if (!existingProfile) {
        const preview = text ? ` — "${text.slice(0, 80)}"` : '';
        notifyAdmin(`New Randi user: ${from}${preview}`);
      }
    ...
    async (from, text) => {
      // Blocked sender tried to reach Randi — notify admin
      const preview = text ? ` — "${text.slice(0, 80)}"` : '';
      notifyAdmin(`Randi access request from ${from}${preview}`);
    }
  )
);
...
║  GET  /usage/:tenantId — Token usage per tenant      ║
║                                                      ║
